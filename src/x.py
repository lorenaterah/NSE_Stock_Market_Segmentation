...
# First 6 features used exclusively for risk label assignment
GROUPING_FEATURES = [
    "std_return", "volatility_mean", "volatility_max",
    "max_drawdown", "downside_deviation", "var_95",
]
...
def assign_cluster_labels(df_clustered, method_col="KMeans_Cluster", features=None):
    """Assign human-readable risk labels to clusters using automatic score-based assignment.

    Clustering is performed on CLUSTERING_FEATURES (all 15), but labels are
    derived from GROUPING_FEATURES (first 6 core volatility / downside-risk
    features) so that the risk tiers reflect only those key dimensions.
    """
